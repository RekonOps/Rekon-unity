...
```
https://github.com/GaoZombie/Bug-OneTouch-unity.git
```

### 2단계: Settings 생성

`Assets` 메뉴 → `Create` → `Bug-OneTouch` → `Settings`를 선택하여 `BugOneTouchSettings` 에셋을 생성합니다.

생성된 에셋을 Project Settings의 Bug-OneTouch 항목에 할당합니다.
...
```
https://github.com/GaoZombie/Bug-OneTouch-unity.git#v0.1.0
```

### .unitypackage로 설치

1. [GitHub Releases](https://github.com/GaoZombie/Bug-OneTouch-unity/releases) 페이지에서 최신 `.unitypackage` 파일 다운로드
...
```
git clone https://github.com/GaoZombie/Bug-OneTouch-unity.git
```
...
YourUnityProject/
  Packages/
    com.gaozombie.bug-onetouch/   ← 여기에 복사
```
...
2. 우클릭 → `Create` → `Bug-OneTouch` → `Settings` 선택
3. 파일명은 `BugOneTouchSettings`로 유지하거나 원하는 이름으로 변경

### Project Settings에 등록

1. `Edit` 메뉴 → `Project Settings` → `Bug-OneTouch` 항목 선택
...
핫키는 Play Mode에서만 작동합니다. 에디터 모드에서는 Bug-OneTouch 에디터 윈도우를 통해 수동으로 캡처할 수 있습니다.
...
`BugOneTouchSettings` 에셋의 `Auth Broker URL` 필드에 Auth Broker 서버 주소를 입력합니다:
...
`Project Settings` → `Bug-OneTouch` → `Jira 연결` 버튼을 클릭합니다.

**3단계: 브라우저 인증**

자동으로 브라우저가 열리고 Jira OAuth 인증 페이지로 이동합니다. Atlassian 계정으로 로그인하고 Bug-OneTouch 앱의 권한 요청을 승인합니다.
...
- `Default Labels`: 자동으로 추가할 레이블 설정 (기본값: `bug-onetouch-unity`, `unity`)
...
Bug-OneTouch가 자동으로 다음 항목을 수집합니다:
...
`Window` 메뉴 → `Bug-OneTouch` → `Bundle Manager`를 선택하여 번들 관리 창을 엽니다.
...
Bug-OneTouch는 Play Mode 진입 시 자동으로 이전 세션의 비정상 종료를 감지합니다. 에디터 크래시, 강제 종료, 또는 비정상 종료가 감지되면 세션 시작 시 크래시 복구 윈도우가 표시됩니다.
...
// 게임 데이터를 컨텍스트에 추가
BugOneTouchContext.Add("current_level", "5");
BugOneTouchContext.Add("player_hp", playerHp.ToString());
BugOneTouchContext.Add("scene_name", SceneManager.GetActiveScene().name);
...
BugOneTouch.Instance.ContextRegistry.Register(provider);

// 게임 종료 시 해제
BugOneTouch.Instance.ContextRegistry.Unregister(provider);
```
...
작성한 파일 경로를 `BugOneTouchSettings`의 `Masking Rules Path`에 입력합니다.
...
1. Play Mode인지 확인합니다. Bug-OneTouch는 에디터 모드에서 핫키가 작동하지 않습니다.
2. `BugOneTouchSettings` 에셋이 Project Settings에 올바르게 할당되어 있는지 확인합니다.
...
1. `BugOneTouchSettings`에서 `Video Enabled`가 체크되어 있는지 확인합니다.
2. Play Mode 진입 후 충분한 시간(최소 몇 초)이 지난 후 캡처했는지 확인합니다. 링 버퍼가 채워지기 전에 캡처하면 영상이 없을 수 있습니다.
3. Unity 에디터 Console 창에서 `[BugOneTouch]` 관련 오류 메시지를 확인합니다.
...
2. `BugOneTouchSettings`의 Crash Recovery 설정이 활성화되어 있는지 확인합니다.
...
- [GitHub Issues](https://github.com/GaoZombie/Bug-OneTouch-unity/issues) - 버그 리포트 및 기능 요청
