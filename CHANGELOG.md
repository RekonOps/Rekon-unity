...
첫 공개 릴리스 (GitHub Public + OpenUPM 등록 대상).

### 변경
- **라이선스 변경**: Apache License 2.0 → **MIT License**. Unity SDK 는 유입 채널이므로 도입 마찰을 최소화하는 것이 목적 — 동종 SDK(Sentry / Bugsnag / Backtrace Unity) 가 모두 MIT 이며, Apache 의 NOTICE 전파 의무(§4)가 게임 빌드 배포자에게 부담이 됨. 수익화 지점은 웹/백엔드에 있으므로 SDK 라이선스로 보호할 대상이 없음. (백엔드·웹은 별도 비공개 유지)
- **버전 1.0.0**: public 공개 시작에 맞춘 안정 버전 선언. 이후 호환성을 깨는 변경은 major 를 올림.

### 참고
- OpenUPM 에는 **v1.0.0 이상만 게시**됨 (`minVersion: '1.0.0'`). v0.5.1 이하 태그는 Apache 2.0 / BSL 1.1 시절 산출물이라 레지스트리 대상에서 제외.
