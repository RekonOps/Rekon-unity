...
namespace RekonOps.Rekon
{
    ...
    ///
    /// 플래그 파일 경로:
    ///   {persistentDataPath}/Rekon/crash_recovery/abnormal_exit.flag
    ///
    /// 주의:
    ...
    public class AbnormalExitDetector
    {
        ...
        private static void OnRuntimeInitialize()
        {
            try
            {
                ...
                if (previousCrash)
                {
                    Debug.LogWarning("[Rekon] 이전 세션 비정상 종료 감지! 크래시 번들을 생성합니다.");
                ...
                {
                    Debug.Log("[Rekon] 이전 세션 정상 종료 확인.");
                }
                ...
                Debug.Log($"[Rekon] AbnormalExitDetector 초기화 완료. 플래그: {FlagFilePath}");
            }
            catch (Exception ex)
            {
                // 초기화 실패가 게임 시작을 방해하면 안 됨
                Debug.LogError($"[Rekon] AbnormalExitDetector 초기화 실패: {ex.Message}");
            }
        }
        ...
        public static void CreateFlagFile()
        {
            try
            {
                ...
                Debug.Log($"[Rekon] 비정상 종료 플래그 생성: {FlagFilePath}");
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[Rekon] 비정상 종료 플래그 생성 실패 (무시): {ex.Message}");
            }
        }
        ...
        public static void DeleteFlagFile()
        {
            try
            {
                if (File.Exists(FlagFilePath))
                {
                    ...
                    Debug.Log("[Rekon] 비정상 종료 플래그 삭제 완료 (정상 종료).");
                }
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[Rekon] 비정상 종료 플래그 삭제 실패 (무시): {ex.Message}");
            }
        }
