    ...
    public class JiraMetadataService
    {
        ...
        public async Task<JiraProject[]> GetProjectsAsync(CancellationToken ct = default)
        {
            try
            {
                ...
                Debug.Log($"[BugOneTouch] 프로젝트 API 원문 응답 (처음 500자): {(json.Length > 500 ? json.Substring(0, 500) : json)}");
            ...
            {
                Debug.LogError($"[BugOneTouch] 프로젝트 목록 조회 실패: {ex.Message}");
                throw;
            }
        }
        ...
        public async Task<JiraIssueTypeInfo[]> GetIssueTypesAsync(string projectKey, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                Debug.Log($"[BugOneTouch] 이슈 타입 API 원문 응답 (처음 500자): {(json.Length > 500 ? json.Substring(0, 500) : json)}");
            ...
            {
                Debug.LogError($"[BugOneTouch] 이슈 타입 목록 조회 실패 (프로젝트: {projectKey}): {ex.Message}");
                throw;
            }
        }
            ...
            CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                Debug.Log($"[BugOneTouch] 필드 API 호출: GET {endpoint}");
                string json = await _apiClient.GetAsync(endpoint, ct);
                Debug.Log($"[BugOneTouch] 필드 API 응답 (첫 500자): {(json?.Length > 500 ? json.Substring(0, 500) + "..." : json)}");

                if (string.IsNullOrEmpty(json))
                {
                    Debug.LogError("[BugOneTouch] 필드 API 응답이 비어 있습니다.");
                ...
                {
                    Debug.LogError($"[BugOneTouch] 필드 JSON 파싱 실패: {parseEx.Message}\n응답 내용: {json}");
                ...
                {
                    Debug.LogWarning($"[BugOneTouch] 필드 응답의 'fields'/'values' 필드가 null이거나 비어 있습니다. JSON: {json}");
                ...
                Debug.Log($"[BugOneTouch] 필드 파싱 완료: {items.Length}개 필드");
                return result;
            }
            catch (OperationCanceledException)
            {
            ...
            {
                Debug.LogError(
                    $"[BugOneTouch] 필드 목록 조회 실패 (프로젝트: {projectKey}, 이슈타입: {issueTypeId}): {ex.Message}");
                throw;
            }
        }
        ...
        public async Task<JiraUser> GetMyselfAsync(CancellationToken ct = default)
        {
            try
            {
                ...
                Debug.Log($"[BugOneTouch] /myself 응답 (처음 200자): {(json.Length > 200 ? json.Substring(0, 200) : json)}");
            ...
            {
                Debug.LogError($"[BugOneTouch] 현재 사용자 조회 실패: {ex.Message}");
                throw;
            }
        }
        ...
        public async Task<JiraUser[]> GetAssignableUsersAsync(string projectKey, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                Debug.Log($"[BugOneTouch] assignable users 응답 (처음 300자): {(json.Length > 300 ? json.Substring(0, 300) : json)}");
            ...
            {
                Debug.LogError($"[BugOneTouch] 할당 가능 사용자 조회 실패: {ex.Message}");
                throw;
            }
        }
        ...
        public async Task<JiraBoard[]> GetBoardsAsync(string projectKey, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                Debug.Log($"[BugOneTouch] boards 응답 (처음 300자): {(json.Length > 300 ? json.Substring(0, 300) : json)}");
            ...
            catch (Exception ex)
            {
                ...
                {
                    Debug.LogWarning(
                        "[BugOneTouch] Agile API 접근 권한이 없습니다. " +
                ...
                {
                    Debug.LogWarning($"[BugOneTouch] 보드 조회 실패 (스프린트 조회 불가): {ex.Message}");
                }
        ...
        public async Task<JiraSprint[]> GetSprintsAsync(int boardId, CancellationToken ct = default)
        {
            try
            {
                ...
                Debug.Log($"[BugOneTouch] sprints 응답 (처음 300자): {(json.Length > 300 ? json.Substring(0, 300) : json)}");
            ...
            catch (Exception ex)
            {
                ...
                {
                    Debug.LogWarning($"[BugOneTouch] 스프린트 조회 실패: {ex.Message}");
                }
            ...
            string projectKey, string issueType = null, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                Debug.Log($"[BugOneTouch] search 응답 (처음 300자): {(json.Length > 300 ? json.Substring(0, 300) : json)}");
            ...
            {
                Debug.LogWarning($"[BugOneTouch] 이슈 검색 실패: {ex.Message}");
