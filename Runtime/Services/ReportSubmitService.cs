    ...
    public class ReportSubmitService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                Debug.Log($"[Rekon] 리포트 생성 완료: {reportId} (파일 {createResponse.report_files.Length}개)");
                ...
                foreach (var entry in fileMap)
                {
                    ...
                    {
                        Debug.LogError($"[Rekon] 파일 업로드 실패: {fileInfo.FileName} - {uploadResult.ErrorMessage}");
                    ...
                    Debug.Log($"[Rekon] 파일 업로드 완료: {fileInfo.FileName} ({completedFiles}/{totalFiles})");
                }
                ...
                if (!string.IsNullOrEmpty(confirmResponse.warning))
                    Debug.LogWarning($"[Rekon] confirm-upload 경고: {confirmResponse.warning}");
                ...
                if (confirmResponse.results != null)
                {
                    foreach (var result in confirmResponse.results)
                    {
                        if (result.status == "confirmed" || result.status == "r2_skipped")
                            ...
                            Debug.LogWarning($"[Rekon] 파일 확인 실패: {result.file_id} -> {result.status}");
                    }
                }

                Debug.Log($"[Rekon] 업로드 확인 완료: {confirmedCount}/{uploadedFileIds.Count}개 파일 확인됨");
            ...
            catch (UsageLimitExceededException ex)
            {
                ...
                Debug.LogWarning($"[Rekon] 사용량 한도 초과: reason={ex.LimitReason}, upgradeUrl={ex.UpgradeUrl}");
            ...
            {
                Debug.Log("[Rekon] 리포트 제출이 취소되었습니다.");
            ...
            CancellationToken ct)
        {
            ...
            {
                Debug.LogError($"[Rekon] 리포트 생성 응답 파싱 실패. 응답: {responseJson}");
            ...
            while (attempt < MaxRetries)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (attempt < MaxRetries)
                    {
                        ...
                        Debug.LogWarning($"[Rekon] API 요청 실패 (시도 {attempt}/{MaxRetries}), " +
        ...
        {
            Debug.LogError($"[Rekon] {errorMessage}");
            return new SubmitResult
            {
                Success = false,
                ReportId = reportId,
