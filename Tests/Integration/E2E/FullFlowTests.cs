...
namespace RekonOps.Rekon.Tests.Integration
{
    ...
    public class FullFlowTests
    {
        ...
        public void SetUp()
        {
            ...
            _tempDir = Path.Combine(Path.GetTempPath(), "Rekon_E2E_" + Guid.NewGuid().ToString("N")[..8]);
        ...
        public async Task Jira_제출_서비스_정상_제출_상태_전이_검증()
        {
            ...
            var result = await mockJiraFacade.SubmitAsync("bundle-test-001", "TEST", "[Rekon] E2E 테스트 이슈");
        ...
        public async Task Jira_이슈_생성_실패시_Failed_상태_전이()
        {
            ...
            try
            {
                var result = await mockJiraFacade.SubmitAsync("bundle-fail-001", "TEST", "[Rekon] 이슈 생성 실패 테스트");
            }
            catch (Exception ex)
            {
