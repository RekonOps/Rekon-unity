#!/usr/bin/env bash
# validate-package.sh
# Bug-OneTouch UPM 패키지 검증 스크립트
#
# 기능:
#   1. package.json 필수 필드 검증
#   2. Assembly Definition (.asmdef) 참조 무결성 검증
#   3. 네임스페이스 일관성 검증 (GaoZombie.BugOneTouch)
#
# 사용법:
...
EXPECTED_NAMESPACE="GaoZombie.BugOneTouch"
NAMESPACE_ERRORS=0
...
if [ ${#CS_FILES[@]} -eq 0 ]; then
    ...
    for cs_file in "${CS_FILES[@]}"; do
        ...
        if ! grep -q "namespace $EXPECTED_NAMESPACE" "$cs_file" 2>/dev/null; then
            # 서브네임스페이스(예: GaoZombie.BugOneTouch.Samples)는 허용
            if ! grep -q "namespace ${EXPECTED_NAMESPACE}\." "$cs_file" 2>/dev/null; then
...
REQUIRED_FILES=(
    ...
    "CHANGELOG.md"
    "LICENSE"
    "Runtime/BugOneTouch.Runtime.asmdef"
    "Editor/BugOneTouch.Editor.asmdef"
)
