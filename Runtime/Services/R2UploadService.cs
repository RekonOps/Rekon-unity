            ...
            CancellationToken cancellationToken = default)
        {
            ...
            while (attempt < MaxRetries)
            {
                ...
                try
                {
                    if (attempt > 0)
                    {
                        Debug.Log($"[BugOneTouch] R2 업로드 재시도 {attempt}/{MaxRetries}");
                    }

                    var result = await PutUploadAsync(
                        presignedUrl, fileData, contentType, progress, cancellationToken);

                    Debug.Log($"[BugOneTouch] R2 업로드 성공 ({fileData.Length / 1024}KB)");
                    return result;
                }
                catch (OperationCanceledException)
                {
                    Debug.Log("[BugOneTouch] R2 업로드 취소됨");
                    throw;
                }
                catch (R2UploadException ex)
                {
                    ...
                    {
                        Debug.LogError(
                            $"[BugOneTouch] R2 업로드 클라이언트 오류 (HTTP {ex.StatusCode}), 재시도하지 않음: {ex.Message}");
                        break;
                    }
                    ...
                    if (attempt < MaxRetries)
                    {
                        ...
                        Debug.LogWarning(
                            $"[BugOneTouch] R2 업로드 실패 (시도 {attempt}/{MaxRetries}), " +
            ...
            Debug.LogError($"[BugOneTouch] R2 업로드 최종 실패 (재시도 {MaxRetries}회 초과): {lastException?.Message}");
