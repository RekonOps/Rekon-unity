...
namespace GaoZombie.BugOneTouch.Editor
{
    ...
    /// Window/Bug-OneTouch/Bundles 메뉴에서 열립니다.
    ///
    /// 기능:
    ...
    public class BundleManagerWindow : EditorWindow
    {
        ...
        [MenuItem(BugOneTouchEditorInfo.MenuRoot + "/Bundles")]
        ...
        internal async Task RefreshBundlesAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[BugOneTouch] BundleManagerWindow 로딩 오류: {ex}");
            }
            finally
            {
                _isLoading = false;
        ...
        private async Task SubmitBundleToJiraAsync(BundleManifest bundle)
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[BugOneTouch] 번들 제출 준비 오류: {ex}");
            }
        }
        ...
        private async Task RetrySubmitAsync(BundleManifest bundle)
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[BugOneTouch] 재시도 준비 오류: {ex}");
            }
        }
        ...
        private async Task DeleteBundleAsync(BundleManifest bundle)
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[BugOneTouch] 번들 삭제 오류: {ex}");
            }
        }
        ...
        private async Task ApplyRetentionPolicyAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[BugOneTouch] 보관 정책 오류: {ex}");
            }
        }
        ...
        /// </summary>
        private async Task<int> ApplyRetentionInternal(BugOneTouchSettings settings)
        {
            int deleted = 0;
            ...
            while (all.Count > settings.maxBundles)
            {
                ...
                all.RemoveAt(0);
                deleted++;
                Debug.Log($"[BugOneTouch] 보관 정책 삭제(개수 초과): {oldest.id}");
            }
            ...
            while (totalBytes > maxBytes && all.Count > 0)
            {
                ...
                all.RemoveAt(0);
                deleted++;
                Debug.Log($"[BugOneTouch] 보관 정책 삭제(용량 초과): {oldest.id}");
            }
