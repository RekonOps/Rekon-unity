    ...
    public class CrashRecoveryFlowTests
    {
        ...
        public void SetUp()
        {
            ...
            string testRoot = Path.Combine(
                Path.GetTempPath(),
                "Rekon_CrashRecovery_" + Guid.NewGuid().ToString("N")[..8]);

            _tempCrashRecoveryDir = Path.Combine(testRoot, "Rekon", "crash_recovery");
            _tempActiveDir = Path.Combine(_tempCrashRecoveryDir, "active");
            _tempCrashBundlesDir = Path.Combine(testRoot, "Rekon", "crash_bundles");
