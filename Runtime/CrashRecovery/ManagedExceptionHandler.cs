...
namespace RekonOps.Rekon
{
    ...
    public class ManagedExceptionHandler : IDisposable
    {
        ...
        public ManagedExceptionHandler(CrashBundleWriter bundleWriter)
        {
            ...
            Debug.Log("[Rekon] ManagedExceptionHandler 초기화 완료. Exception 감지 시작.");
        }
        ...
        public void Dispose()
        {
            ...
            Debug.Log("[Rekon] ManagedExceptionHandler 정리 완료.");
        }
        ...
        private void OnLogReceived(string condition, string stackTrace, LogType logType)
        {
            ...
            if (IsOnCooldown)
            {
                Debug.LogWarning($"[Rekon] Exception 감지 (쿨다운 중, 무시): {condition}");
                return;
            }
            ...
            {
                Debug.LogWarning("[Rekon] Exception 감지 (번들 생성 중, 무시)");
                return;
            }

            Debug.LogWarning($"[Rekon] Managed Exception 감지! 크래시 번들을 생성합니다: {condition}");
        ...
        private async void StartBuildBundle(string exceptionType, string exceptionMessage, string stackTrace)
        {
            ...
            try
            {
                ...
                if (manifest != null)
                {
                    Debug.Log($"[Rekon] 크래시 번들 생성 완료: {manifest.id}");
                    OnCrashBundleCreated?.Invoke(manifest);
                }
                else
                {
                    Debug.LogWarning("[Rekon] 크래시 번들 생성 실패 (null 반환).");
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"[Rekon] 크래시 번들 생성 중 예외: {ex.Message}");
            }
            finally
            {
