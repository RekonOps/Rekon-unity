...
namespace GaoZombie.BugOneTouch
{
    ...
    public class VideoEncoder : IVideoEncoder
    {
        ...
        public async Task EncodeAsync(FrameData[] frames, string outputPath, VideoEncoderConfig config, CancellationToken cancellationToken = default)
        {
            ...
            {
                Debug.LogWarning("[BugOneTouch] VideoEncoder: 인코딩할 프레임이 없습니다.");
                return;
            }
            ...
            try
            {
                ...
                Debug.Log($"[BugOneTouch] 영상 인코딩 완료: {outputPath} ({frames.Length}프레임)");
            }
            catch (Exception ex)
            {
                Debug.LogError($"[BugOneTouch] 영상 인코딩 실패: {ex.Message}");
                throw;
            }
        }
