...
Rekon 플러그인의 핵심 기능을 실제 코드로 보여주는 샘플입니다.
...
| `Scripts/SampleBugReporter.cs` | `RekonContext` 정적 API 사용 예제 |
...
5. `Assets/Samples/Rekon/0.1.0/BasicDemo/` 폴더에 파일이 복사됩니다
...
3. 실제 프로젝트에서 `Rekon.Instance.ContextRegistry.Register(this)` 호출 코드 추가
...
```csharp
// 데이터 추가/업데이트
RekonContext.Add("key", "value");

// 데이터 제거
RekonContext.Remove("key");
...
// 등록
Rekon.Instance.ContextRegistry.Register(new MyProvider());
```
